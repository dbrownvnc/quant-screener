...
with st.expander("✨ 앱 소개 및 사용법 (v7.9)"):
    st.markdown('''
    ...
    **알고리즘 로직:**
    ...
    ---
    **v7.9 변경점:**
    1.  **컬럼명 복원:** 분석 결과 테이블의 컬럼명을 이전 버전(v7.7)과 같이 전체 이름으로 복원했습니다.
    2.  **모바일 최적화 유지:** 폰트 크기 축소 등 모바일 가독성을 위한 스타일은 그대로 유지됩니다.
    ''')
